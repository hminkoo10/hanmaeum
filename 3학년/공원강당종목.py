...
def load_base_settings():
    ...
    try:
        ...
        ini_path = os.path.join(base_dir, "설정", "공원강당.ini")
...
def load_event_settings(path):
    ...
    try:
        ...
        if not events:
            raise ValueError("공원강당 설정 파일에 유효한 종목이 없습니다.")
        
        return events
    except Exception as e:
        messagebox.showerror("오류", f"공원강당 설정 파일을 불러오는 중 문제가 발생했습니다.\n{str(e)}")
        exit()
...
if __name__ == "__main__":
    try:
        ...
        RESULTS_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "결과", "공원강당.xlsx")
        create_gui()
    except Exception as e:
